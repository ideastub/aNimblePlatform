<%@ page import="com.nimble.domain.artifact.*;com.nimble.domain.reference.*;com.nimble.utils.view.ViewHelper; com.nimble.utils.domain.ReferenceHelper;com.nimble.controller.artifact.*;com.nimble.domain.release.SprintQuery;" %>
...
<div class="body" style="width:95%;float:left;">
	...
	<%
	request.viewHelper.setSectionTitle('Update ' + viewHelper.getAppControlLabel('section_title'))
	...
	<div class="dialog" style="height:${(viewHelper.getArtifactHeightTotal(viewHelper.getArtifactClassName())*2)+60}px;width:100%;background-color: #FFFFFF;position:relative;">
            ...
            <g:form method="post" name="myform" id="${artifactInstance?.id}" action="update" controller="${viewHelper.getArtifactControllerName()}">
...
<g:if test="${viewHelper.isViewControlActive('artifact_nbr')}">
                            <div class="prop" style="position:absolute;top:${viewHelper.getAppControlVertPos('artifact_nbr')}px;">
                                ...
                                <td class="value ${hasErrors(bean: artifactInstance, field: 'artifact_nbr', 'errors')}">
                                    <g:textField name="artifact_nbr" value="${fieldValue(bean: artifactInstance, field: 'artifact_nbr')}" />
                                </td>
                                </tr></table>
                            </div>
</g:if>
...
<g:if test="${viewHelper.isViewControlActive('sprint')}">
                            <div class="prop" style="position:absolute;top:${viewHelper.getAppControlVertPos('sprint')}px;">
                                ...
                                <td class="value ${hasErrors(bean: artifactInstance, field: 'sprint', 'errors')}">
                                   <g:select name="sprint.id" from="${com.nimble.domain.release.SprintQuery.getAllSprintsForProject(session?.curProject?.id)}" optionKey="id" value="${artifactInstance?.sprint?.id}"  noSelection="['':'- Select -']"/>
                                </td>
                                </tr></table>                                
                            </div>
</g:if>
...
<g:if test="${viewHelper.isViewControlActive('release')}">
                            <div class="prop" style="position:absolute;top:${viewHelper.getAppControlVertPos('release')}px;">
                                ...
                                <td class="value ${hasErrors(bean: artifactInstance, field: 'release', 'errors')}">
                                   <g:select name="release.id" from="${com.nimble.domain.release.ReleaseQuery.getAllReleasesForProject(session?.curProject?.id)}" optionKey="id" value="${artifactInstance?.release?.id}"  noSelection="['':'- Select -']"/>
                                </td>
                                </tr></table>                                
                            </div>
</g:if>
                                ...
                                <td class="value ${hasErrors(bean: artifactInstance, field: 'description', 'errors')}">
                                    ...
                                    --%>
                                    			<textarea id="description" name="description" rows="15" cols="80" style="width: 80%">
													${artifactInstance?.description.encodeAsHTML()}
												</textarea>
